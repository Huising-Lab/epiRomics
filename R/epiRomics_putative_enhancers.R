...
#' @noRd
.collect_histone_source <- function(chromatin_states, epiRomics_dB,
                                     enhancer_states) {
...
#' @return list with gr (GRanges or NULL) and labels (character or NULL)
#' @noRd
.collect_tf_source <- function(epiRomics_dB, annot_by_type, db_prefix) {
...
#' @noRd
.annotate_histone_marks <- function(merged, epiRomics_dB, annot_by_type,
                                     db_prefix) {
  n_merged <- base::length(merged)
...
#'   (integer)
#' @noRd
.annotate_tf_cobinding <- function(merged, epiRomics_dB, annot_by_type,
                                    db_prefix) {
  n_merged <- base::length(merged)
...
epiRomics_putative_enhancers <- function(epiRomics_dB,
  ...
  hist_result <- .collect_histone_source(chromatin_states, epiRomics_dB,
    enhancer_states)
  ...
  tf_result <- .collect_tf_source(epiRomics_dB, annot_by_type, db_prefix)
  ...
  hist_annot <- .annotate_histone_marks(merged, epiRomics_dB, annot_by_type,
    db_prefix)

  # ANNOTATE: TF co-binding
  tf_annot <- .annotate_tf_cobinding(merged, epiRomics_dB, annot_by_type,
    db_prefix)
...
#' @param pe_gr GRanges of putative enhancer regions.
#' @param n_pe Integer. Number of putative enhancers.
#' @param epiRomics_track_connection data.frame or NULL. BigWig track sheet.
...
.filter_by_signal <- function(putative_enhancers, pe_gr, n_pe,
                               epiRomics_track_connection, mode,
                               signal_threshold) {
...
epiRomics_filter_accessible <- function(putative_enhancers,
  ...
  sig_result <- .filter_by_signal(putative_enhancers, pe_gr, n_pe,
    epiRomics_track_connection, mode, signal_threshold)
