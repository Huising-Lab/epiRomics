...
#' Used internally by \code{plot_tracks()} to load ATAC, RNA,
...
.import_bw_group_data <- function(
  ...
  for (j in base::seq_len(n)) {
    bw <- base::tryCatch(
      rtracklayer::import(
        .bw_path_for_rtracklayer(tc_sub[j, 1]),
        format = "BigWig",
        selection = rtracklayer::BigWigSelection(region_gr)),
      error = function(e) base::stop(
        "track ", j, " failed (", tc_sub[j, 1], "): ",
...
#' Validate inputs for plot_tracks
#'
#' Checks putative_enhanceosome, index, database,
#' track_connection, and layout parameters. Returns validated
#' signal_style and signal_layout after match.arg.
#'
#' @param putative_enhanceosome epiRomics S4 object
#' @param index numeric row index
#' @param database epiRomics S4 database object
#' @param track_connection data.frame of track connections
#' @param keep_epitracks logical
...
#' @noRd
.validate_track_layer_inputs <- function(putative_enhanceosome,
                                         index,
                                         database,
                                         track_connection,
                                         keep_epitracks,
  ...
  validate_numeric_param(
    index, "index",
    "plot_tracks", min_val = 1
  )
  validate_database(
    database, "plot_tracks"
  )
  ...
  if (!base::is.data.frame(track_connection)) {
    base::stop(
      "track_connection must be a data frame"
    )
  }
  n_annot <- base::length(
    putative_enhanceosome@annotations
  )
  if (index > n_annot) {
    base::stop(base::sprintf(
      "index (%d) exceeds annotations (%d)",
      index, n_annot
    ))
  }
...
#' @noRd
.extract_focal_region <- function(putative_enhanceosome,
                                  index,
                                  database,
                                  gene_lookup) {
  enh <- putative_enhanceosome@annotations[index]
...
.prepare_signal_data <- function(
    track_connection, region_gr,
    xlims, mirror, signal_layout,
    show_bigwig,
    quantile_cap = 0.98,
    scale_factor = 1.1) {
  tc_atac <- track_connection[
    track_connection$type == "atac",
    , drop = FALSE
  ]
  tc_rna <- track_connection[
    track_connection$type == "rna",
    , drop = FALSE
  ]
  tc_hist <- track_connection[
    track_connection$type == "histone",
    , drop = FALSE
  ]
...
#' @noRd
.assemble_annotation_panels <- function(enh_save, database,
                                        keep_epitracks, enh,
                                        enh_id, index,
                                        gene_lookup,
  ...
  if (keep_epitracks) {
    ...
    region_annots <- IRanges::subsetByOverlaps(
      database@annotations, region_gr
    )
    region_types <- region_annots$type
    db_prefix <- base::paste0(genome, "_custom_")
    meta <- enh_save@meta
    all_types <- base::unique(database@annotations$type)
    ...
    if (show_chromatin && !base::is.null(chromatin_states) &&
      ...
      # strand regions — expected for unstranded TxDb entries.
      ctx_tss <- GenomicRanges::trim(base::suppressWarnings(
        GenomicRanges::resize(region_genes, width = 1L, fix = "start")))
      # Suppress same upstream warning for the TSS-window expansion step.
...
#' @noRd
.resolve_gene_coordinates <- function(gene_symbol, database) {
  # Resolve TxDb
  txdb_obj <- resolve_txdb(database@txdb)
  # Resolve OrgDb without requiring the package to be attached.
  # The org.* packages export an object of the same name as the package
  # (e.g. org.Hs.eg.db::org.Hs.eg.db). getExportedValue() retrieves it
  # from a loaded namespace — requireNamespace() is sufficient; library()
  # is not needed, matching the Suggests-tier contract.
  orgdb <- base::tryCatch({
    if (!base::requireNamespace(database@organism, quietly = TRUE)) {
      base::stop("Package not installed: ", database@organism)
    }
    base::getExportedValue(database@organism, database@organism)
  }, error = function(e) {
    base::stop("Cannot resolve organism database: ",
               database@organism, " (",
...
#'
#' @param putative_enhanceosome epiRomics class
#'   database containing putative enhanceosome calls
#' @param index numeric of row value from
#'   putative_enhanceosome to visualize
#' @param database epiRomics class database containing
#'   all data initially loaded
#' @param track_connection data frame containing
#'   bigwig track locations and their names
#' @param keep_epitracks logical indicating whether
#'   to show enhancer and chip tracks, default is TRUE
#' @param chromatin_states data.frame, optional output from
#'   \code{\link{classify_chromatin_states}}. When provided, the enhancer
...
#'   \code{\link{plot_gene_tracks}} to display a gene locus without
...
#'   )
#' }
plot_tracks <- function(putative_enhanceosome,
                                        index,
                                        database,
                                        track_connection,
                                        keep_epitracks = TRUE,
                                        ...
                                        width = 10,
                                        height = 8) {
  v <- .validate_track_layer_inputs(putative_enhanceosome,
    index, database, track_connection,
    keep_epitracks, signal_style, signal_layout, export)
  ...
  focal <- .extract_focal_region(putative_enhanceosome,
    index, database, gene_lookup)
  hl_fn <- function() .draw_highlight(focal$enh_s, focal$enh_e,
    "#F0E6F680", "#D8BFD8", gene_lookup)
  txdb_obj <- resolve_txdb(database@txdb)
  orgdb <- base::tryCatch(
    base::get(database@organism),
    error = function(e) NULL)
  ...
  sig <- .prepare_signal_data(
    track_connection, gm$region_gr,
    gm$xlims, mirror, signal_layout,
    show_bigwig, quantile_cap, scale_factor)
  ap <- .assemble_annotation_panels(putative_enhanceosome,
    database, keep_epitracks, focal$enh, focal$enh_id,
    index, gene_lookup, show_enhancer_highlight, show_chromatin,
...
#' @rdname plot_tracks
#' @export
plot_tracks_fast <- plot_tracks


#' Visualize any gene locus with multi-track BigWig overlay
#'
#' A convenience wrapper around \code{\link{plot_tracks}} that
...
#' \code{plot_tracks} for rendering. The viewing window spans
...
#' @param track_connection A data.frame from the BigWig CSV
#'   sheet (columns: path, name, color, type).
#' @param chromatin_states Optional. Output of
#'   \code{\link{classify_chromatin_states}}.
...
#'   )
#' }
plot_gene_tracks <- function(gene_symbol,
                                        database,
                                        track_connection,
                                        chromatin_states = NULL,
                                        padding = 1000L,
  ...
  validate_database(database, "plot_gene_tracks")
  if (!base::is.data.frame(track_connection))
    base::stop("track_connection must be a data frame")

  # Resolve gene coordinates and build synthetic epiRomics object
  gc <- .resolve_gene_coordinates(gene_symbol, database)
  ...
  synth_obj@meta <- database@meta; synth_obj@txdb <- database@txdb
  synth_obj@organism <- database@organism
  synth_obj@genome <- database@genome

  plot_tracks(synth_obj, index = 1L,
    database = database,
