...
epiRomics_enhanceosome <- function(epiRomics_putative_enhancers, epiRomics_dB) {
  ...
  for (c in 1:base::length(epiRomics_chips)) {
    epiRomics_enhanceosome_data_table[, c] <- GenomicRanges::countOverlaps(
  ...
  epiRomics_enhanceosome_data_table <- base::cbind(epiRomics_enhanceosome_data_table,
    ChIP_Hits = base::rowSums(epiRomics_enhanceosome_data_table),
