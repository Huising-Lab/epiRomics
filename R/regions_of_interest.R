...
#'   \code{test_regions} is NULL. One of:
#'   \describe{
#'     \item{\code{"granges"}}{Use \code{test_regions} directly
...
get_regions_of_interest <- function(putative_enhanceosome,
  ...
  # --- Core validation ---
  validate_database(putative_enhanceosome,
    "get_regions_of_interest")

  if (base::length(putative_enhanceosome@annotations) == 0) {
    base::stop("putative_enhanceosome has no annotations")
  }

  annotations <- putative_enhanceosome@annotations

  # --- Backward-compatible: if GRanges passed directly, use it ---
  if (!base::is.null(test_regions)) {
    validate_genomic_ranges(test_regions,
      "get_regions_of_interest")
    return(.roi_filter(putative_enhanceosome,
      test_regions))
  }
  ...
  if (input_type == "granges") {
    base::stop(
      "test_regions (GRanges) required for input_type='granges'"
    )
  }
  ...
  if (input_type == "bed") {
    ...
    return(.roi_filter(putative_enhanceosome, test_gr))
  }
  ...
  if (input_type == "genelist") {
    ...
    return(.roi_from_genelist(putative_enhanceosome, gene_list))
  }

  if (input_type == "combined") {
    return(.roi_combined_filter(putative_enhanceosome, annotations,
      bed_path, gene_list))
  }
}
...
#'
#' @param putative_enhanceosome epiRomicsS4 object.
...
#' @noRd
.roi_combined_filter <- function(putative_enhanceosome, annotations,
                                  bed_path, gene_list) {
