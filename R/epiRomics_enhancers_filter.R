...
#' @param epiRomics_type epiRomics reference containing
...
#'   genome = "hg38")
#' tryCatch(
#'   epiRomics_enhancers_filter(db, db),
#'   error = function(e) message(e$message)
#' )
#' \donttest{
#' filtered <- epiRomics_enhancers_filter(
...
epiRomics_enhancers_filter <- function(
  ...
  if (base::is.null(epiRomics_type)) {
    epiRomics_type <- base::paste0(
      epiRomics_dB@genome, "_custom_fantom"
    )
  }
  validate_character_param(
    epiRomics_type, "epiRomics_type",
    "epiRomics_enhancers_filter"
  )
  ...
  tryCatch(
    {
      ...
      filter_annotations <- epiRomics_functional[
        epiRomics_functional$type == epiRomics_type, ]
