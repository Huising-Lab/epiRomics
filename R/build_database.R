...
#'
#' @param db_file character string of path to
#'   properly formatted csv file containing epigenetic
#'   data. [See vignette for more details]
#' @param txdb_organism a character string containing
#'   the TxDB associated with your data.
#' @param genome a character string naming the
#'   genome assembly associated with your data (e.g.
#'   \code{"mm10"}, \code{"hg38"}, \code{"rn6"},
#'   \code{"dm6"}). The value must match the assembly
#'   referenced by \code{txdb_organism} / the CSV
#'   manifest's \code{genome} column; \code{epiRomics}
#'   itself is organism-agnostic.
#' @param organism a character string
#'   containing the org.db associated with your data.
#' @param extraCols named character vector of extra
#'   columns to read from chip/histone BED files.
#'   Default is NULL (no extra columns). Set to
#'   \code{c(signal = "numeric", pval = "numeric",
#'   qval = "numeric", peak = "numeric")}
...
#'   \code{\link{cache_data}} where the CSV uses relative paths.
...
build_database <-
  function(db_file,
           ...
           extraCols = NULL,
           data_dir = NULL) {
    validate_file_paths(db_file, "build_database")
    validate_character_param(
      txdb_organism, "txdb_organism",
      "build_database"
    )
    validate_character_param(
      genome, "genome",
    ...
    tryCatch(
      {
        ...
        if (base::requireNamespace("data.table", quietly = TRUE)) {
          epiRomics_db_data_table <- base::as.data.frame(
            data.table::fread(db_file, stringsAsFactors = FALSE),
            stringsAsFactors = FALSE
          )
        } else {
          epiRomics_db_data_table <- utils::read.csv(db_file,
                                                      stringsAsFactors = FALSE)
        }
        ...
        missing_cols <- required_cols[
          !(required_cols %in%
              base::colnames(epiRomics_db_data_table))
        ]
        ...
        if (!base::is.null(data_dir)) {
          validate_character_param(data_dir, "data_dir", "build_database")
          ...
          is_relative <- !base::grepl(
            "^(/|[A-Za-z]:)",
            epiRomics_db_data_table$path
          )
        ...
        validate_file_paths(epiRomics_db_data_table$path, "build_database")

        if (!base::all(epiRomics_db_data_table$genome == genome)) {
          base::stop(base::sprintf(
            "Genome mismatch. Expected '%s' but found: %s",
            genome,
            base::paste(
              base::unique(
                epiRomics_db_data_table$genome
              ),
              collapse = ", "
            )
          ))
        ...
        if (base::length(bad_format) > 0L) {
          base::stop(base::sprintf(
            "Empty or NA format in row %d",
            bad_format[1L]
          ))
        }

        # --- Read annotations (Map — annotatr_cache is shared state) ---
        # Pre-compute which rows need extraCols
        needs_extra <- !base::is.null(extraCols) &
          format_vals %in% base::c("chip", "histone")

        extra_list <- base::lapply(
          needs_extra,
          function(ne) if (ne) extraCols else NULL
        )
        base::Map(
          function(path, genome, name, fmt, extra) {
        ...
        epiRomics_db_annot_list <- base::c(
          annotatr::builtin_annotations()[
            annotatr::builtin_annotations() %like%
              genome
          ],
        ...
        if (base::length(epiRomics_db_annot_list) == 0L) {
          base::stop(base::sprintf(
            "No annotations found for genome '%s'",
            genome
          ))
        }

        database <- methods::new("epiRomicsS4")
        ...
        cache_file <- base::file.path(
          cache_dir,
          base::paste0("annotations_", cache_key, ".rds")
        )

        if (base::file.exists(cache_file)) {
          base::message("Loading cached annotations...")
          database@annotations <- base::readRDS(cache_file)
        } else {
          database@annotations <-
            annotatr::build_annotations(
              genome = genome,
              annotations = epiRomics_db_annot_list
            )
          base::saveRDS(database@annotations, cache_file)
        }

        if (base::length(database@annotations) == 0L) {
          base::stop("Failed to build annotations - no data returned")
        }

        database@meta <- epiRomics_db_data_table
        database@txdb <- txdb_organism
        database@organism <- organism
        database@genome <- genome
        base::return(database)
      },
      error = function(e) {
        base::stop(base::sprintf("build_database: %s", e$message))
      }
    )
  }
