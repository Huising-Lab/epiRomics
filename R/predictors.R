...
#'
#' @param enhanceosome epiRomics class database
...
#'   }
#' @export
#' @seealso \code{\link{analyze_tf_overlap}} for overlap fractions without
...
analyze_tf_cobinding <- function(enhanceosome, database,
                                    fdr_threshold = 0.05, min_regions = 5L,
                                    method = c("fisher", "permutation"),
                                    n_permutations = 1000L) {
  validate_database(enhanceosome, "analyze_tf_cobinding")
  validate_database(database, "analyze_tf_cobinding")
  ...
  tf_names <- database@meta[database@meta$type == "chip", "name"]
  ...
  regions <- enhanceosome@annotations
