...
#' @return GViz plot
#' @export
epiRomics_region_tracks <- function(epiRomics_region, epiRomics_dB, epiRomics_track_connection) {
  ...
  range_max <- maxCovFiles(epiRomics_track_connection_chromatin[, 1], epiRomics_region)
  ...
  for (i in 1:base::dim(epiRomics_track_connection_chromatin)[1]) {
    chromatin_tracks <- c(chromatin_tracks, Gviz::DataTrack(
      ...
      chromosome = chr, strand = epiRomics_region@strand, name = epiRomics_track_connection_chromatin[i, 2]
    ))
  }
  if (table(epiRomics_track_connection$type == "rna")[[TRUE]] > 0) {
    ...
    range_max <- maxCovFiles(epiRomics_track_connection_rna[, 1], epiRomics_region)
    max_cov <- plyr::round_any((range_max$X + (range_max$X * 0.1)), accuracy = 10, f = ceiling)
    for (i in 1:base::dim(epiRomics_track_connection_rna)[1]) {
      rna_tracks <- c(rna_tracks, Gviz::DataTrack(
        ...
        chromosome = chr, strand = epiRomics_region@strand, name = epiRomics_track_connection_rna[i, 2]
      ))
    }
  }
