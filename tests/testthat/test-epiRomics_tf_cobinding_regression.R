## Regression baseline tests for epiRomics_tf_cobinding
##
...
test_that("tf_cobinding regression: fisher method", {
  ...
  # We run epiRomics_enhanceosome first to get proper input.
  ...
  enhanceosome <- base::suppressWarnings(base::suppressMessages(
    epiRomics_enhanceosome(pe_dB, dB)
  ))

  # epiRomics_tf_cobinding requires at least 2 ChIP TFs in the dB
  # (which make_synthetic_dB_full provides: TF1, TF2)
  result <- base::suppressWarnings(base::suppressMessages(
    epiRomics_tf_cobinding(enhanceosome, dB, method = "fisher")
  ))
