# Regression baseline tests for build_database()
...
test_that("build_dB regression: synthetic CSV + BED", {
  ...
  # Call build_database with synthetic data
  # This will call annotatr::read_annotations + annotatr::build_annotations
  # Expected benign noise:
  #   * annotatr "No seqlevels in common" when the builtin annotations are
  #     intersected with our synthetic chr1-only BED content.
  #   * ChIPseeker "peak has no overlap with gene" for synthetic peaks that
  #     fall outside real hg38 gene bodies.
  # The regression assertion captures the custom-annotation snapshot, not
  # the warning text.
