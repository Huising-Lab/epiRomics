## Regression baseline tests for analyze_tf_overlap
##
...
test_that("tf_overlap regression: default", {
  ...
  # analyze_tf_overlap takes two epiRomicsS4 dBs and computes pairwise

  # overlap between their ChIP TF annotations. Using the same dB for both
  # gives deterministic self-overlap results.
  # Expected benign noise: analyze_tf_overlap emits progress messages and
  # GenomicRanges may warn about identical inputs during self-overlap. The
  # assertions below concern matrix shape + diagonal values, not noise.
...
test_that("tf_overlap regression: with custom regions", {
  ...
  # Expected benign noise: analyze_tf_overlap progress messages when
  # restricting to a regions= subset; not under test.
  result <- base::suppressWarnings(base::suppressMessages(
    analyze_tf_overlap(dB, dB, regions = custom_regions)
  ))
